...
def dRNN(cell, inputs, rate, scope='default'):
    ...
    print1 = "Building layer: %s, input length: %d, dilation rate: %d, input dim: %d." % (
    ...
    if not EVEN:
        ...
        print2= "=====> %d time points need to be padded. " % (
            dialated_n_steps * rate - n_steps)
        print(print2)
        print3 = "=====> Input length for sub-RNN: %d" % (dialated_n_steps)
        print(print3)
        for i_pad in range(dialated_n_steps * rate - n_steps):
            inputs.append(zero_tensor)
    else:
        dialated_n_steps = n_steps // rate
        print4 =  "=====> Input length for sub-RNN: %d" % (dialated_n_steps)
        print(print4)
