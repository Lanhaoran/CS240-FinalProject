...
def load_data(filename):
    '''
    If no the paramater pass through is a class in UCR database, just fecth it
    '''
    
    if ',' in filename:
        class_list = datasets.ucr_dataset_list()
        #print(class_list)
        dataset_name, type = filename.split(',')
        #print(dataset_name,type)
        assert (dataset_name in class_list)
        (data_train, data_test, target_train, target_test) = datasets.fetch_ucr_dataset(dataset_name,return_X_y=True)
        if type == 'train': return data_train,target_train
        else: return data_test[:23],target_test[:23]
    else:
...
def evaluation(prediction, label):
    ...
    print1 = "acc: %s, nmi: %s, ari: %s, ri: %s" % (acc, nmi, ari, ri)
    print(print1)
    return ri, nmi, acc
...
def cluster_acc(y_true, y_pred):
    ...
    from scipy.optimize import linear_sum_assignment as liner_assignment
    ind = liner_assignment(w.max() - w)
...
# Plot the line chart of the loss during training epoch
def plot_loss(loss):
    x_ = range(len(loss)) # x_axis
    
    plt.plot(x_,loss)

    #x_axis ticks
    #_x_lables = ["{}epoch".format(i) for i in x_]
    #plt.xticks(x_,_x_lables)
    #labels
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
